...
from .ie_func_bank import IEFuncBank
...
class EoH:
    def __init__(self,
        ...
        Args:
            ...
            **kwargs                    : some args pass to 'llm4ad.base.SecureEvaluator'. Such as 'fork_proc'.
                                          Also supports EoH-IE args: 'ie_top_k', 'ie_bank_max_size',
                                          'ie_extract_per_generation', 'ie_init_from_population'.
        """
        ...
        # dynamic IE function bank parameters
        self._ie_top_k = int(kwargs.pop('ie_top_k', 5))
        self._ie_bank_max_size = int(kwargs.pop('ie_bank_max_size', 20))
        self._ie_extract_per_generation = int(kwargs.pop('ie_extract_per_generation', 2))
        self._ie_init_from_population = bool(kwargs.pop('ie_init_from_population', True))
        ...
        # IE function bank state
        self._ie_bank = IEFuncBank()
        self._ie_lock = Lock()
        self._last_ie_update_generation = -1
    ...
    def _sample_evaluate_register(self, prompt):
        ...
        # update IE bank usage credit from this sampled function
        try:
            with self._ie_lock:
                names = self._ie_bank.get_names()
            used = IEFuncBank.extract_called_funcs_from_code(str(func), names)
            with self._ie_lock:
                self._ie_bank.update_usage(used, score, self._population.generation)
        except Exception:
            pass
        ...
        self._maybe_update_ie_bank_after_generation_change()

    def _get_ie_helper_context(self) -> str:
        with self._ie_lock:
            return self._ie_bank.build_prompt_context(self._ie_top_k)

    def _maybe_update_ie_bank_after_generation_change(self):
        """Update helper bank once after each new population generation."""
        gen = self._population.generation
        if gen <= self._last_ie_update_generation:
            return

        with self._ie_lock:
            # double-check with lock for multi-thread safety
            if gen <= self._last_ie_update_generation:
                return
            self._last_ie_update_generation = gen

        self._update_ie_bank_from_elites(gen)

    def _update_ie_bank_from_elites(self, generation: int):
        """Generate new helper funcs from top elites and insert/update IE bank."""
        if generation <= 0 and not self._ie_init_from_population:
            return

        elites = sorted(self._population.population, key=lambda f: f.score, reverse=True)
        elites = [f for f in elites if f is not None and f.score is not None]
        if not elites:
            return

        elite_subset = elites[:max(self._ie_extract_per_generation, 0)]
        added = 0
        for elite in elite_subset:
            try:
                with self._ie_lock:
                    existing_names = self._ie_bank.get_names()
                prompt = EoHPrompt.get_prompt_extract_helper_from_elite(
                    self._task_description_str,
                    elite,
                    existing_names,
                )
                response = self._sampler.llm.draw_sample(prompt)
                helper = self._parse_helper_from_response(response)
                if helper is None:
                    continue
                with self._ie_lock:
                    ok = self._ie_bank.add_or_update(
                        signature=helper['signature'],
                        docstring=helper['docstring'],
                        source_code=helper['source_code'],
                        generation=generation,
                        init_score=float(elite.score) if elite.score is not None else 0.0,
                    )
                    if ok:
                        added += 1
            except Exception:
        ...
        if added > 0:
            with self._ie_lock:
                self._ie_bank.keep_top(self._ie_bank_max_size)

    def _parse_helper_from_response(self, response: str):
        """Parse exactly one helper function from LLM response text."""
        if not response:
            return None
        ...
        while self._continue_loop():
            try:
                ...
                prompt = EoHPrompt.get_prompt_e1(
                    ...
                    helper_context=self._get_ie_helper_context(),
                )
                if self._debug_mode:
