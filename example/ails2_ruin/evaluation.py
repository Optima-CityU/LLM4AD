...
from template import template_program, task_description, aim_java_relative_path, \
...
class Ails2Evaluation(Evaluation):
    ...
    def __init__(self, default_instance_timeout=30,
                 dump_java_output_on_finish: bool = False,
                 jdk_bin_path: str = r";C:\Program Files\Common Files\Oracle\Java\javapath",            # TODO 1
                 max_parallel_instances: int = 7,
                 instances_subdir: str = "XLDemo",  # AILSII 实例子目录
                 **kwargs):
        """
            Args:
    ...
    def run_command(self, commands, python_timeout: int) -> Tuple[str, int]:
        """
        [Refactored] Robustly handles timeouts and ensures stdout is read
        ...
        try:
            # 1. Start the process
            # bufsize=0 (unbuffered) or line buffering can help, but -u in Java isn't standard.
            # We rely on Python reading the pipe.
            process = subprocess.Popen(
                commands,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
                encoding="gbk",  # Ensure this matches your system/Java encoding
            ...
            # 3. Check return code (Normal finish)
            if return_code != 0:
                print(f"Java process failed. Code: {return_code}", file=sys.stderr)
                return "CRASH", return_code

        except subprocess.TimeoutExpired:
            print(f"Java process timed out (> {python_timeout}s). Attempting to capture output...", file=sys.stderr)

            # --- CRITICAL FIX START ---
            # 1. Try to read what's currently in the buffer BEFORE killing
            # This is tricky with Popen, but communicate() usually returns what it got so far in the exception object
            # However, since we are in the except block of communicate, we might not have the data yet.

            # 2. Kill the process
            process.kill()

            # 3. Read the remaining output.
            # communicate() after kill() will read until EOF (which happens immediately after kill)
            try:
                full_stdout, full_stderr = process.communicate(timeout=5)
            except Exception as e:
                print(f"Could not capture final output after kill: {e}", file=sys.stderr)

            return_code = -1  # Mark as timeout
            # --- CRITICAL FIX END ---
        ...
        # Debugging: Uncomment if you suspect empty output
        # if not full_stdout:
        #     print(f"DEBUG: Stdout was empty for {commands[-1]}")

        if full_stdout:
            # Parse lines in reverse to find the last valid score
            lines = full_stdout.strip().split('\n')
            for line in reversed(lines):
                line = line.strip()
                if not line: continue

                # Look for the specific format output by AILSII
                # Usually: "Best solution found: ;12345.0" or just ";12345.0"
                if ';' in line:
                    try:
                        parts = line.split(';')
                        # Take the last part which should be the number
        ...
        if last_line_score:
            if return_code == -1:
                # We got a score despite the timeout
                # print(f"Timeout Recovery: Captured score {last_line_score}")
                pass
            return last_line_score, return_code
        else:
            if return_code == -1:
    ...
    def evaluate(self, java_script: str, subprocess_index=0) -> float | None:
        ...
        if not self.java_commands:
            ...
            for instance_file_path in instance_files:                                                 # TODO 4  如果还是AILSII就不需要改
                ...
                python_timeout = java_limit_time + 25
...
//		only a placeholder, replace with your design
    ...
    eval = Ails2Evaluation(default_instance_timeout=120,dump_java_output_on_finish=True)  # 设置超时
