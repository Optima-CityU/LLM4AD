...
public class AILSII
{
    ...
    public void search() {
        ...
        String baseName = this.instanceName.replace(".vrp", "");

        // REVISION: Add the specific subdirectories to the path
        // Use File.separator to ensure compatibility across Windows/Linux/Mac
        String solutionPath = "CVRPLIB_2025" + java.io.File.separator +
                              "XLDemo" + java.io.File.separator +
                              baseName + ".sol";

        // --- DEBUG CODE START: Check File Path ---
//         System.out.println("\n[DEBUG] --- Starting Search Initialization ---");
//         System.out.println("[DEBUG] Instance Name: " + this.instanceName);
//         System.out.println("[DEBUG] Target Solution Path (Relative): " + solutionPath);

        java.io.File solFile = new java.io.File(solutionPath);
//         System.out.println("[DEBUG] Target Solution Path (Absolute): " + solFile.getAbsolutePath());

//         if (!solFile.exists()) {
//             System.err.println("[DEBUG] ERROR: The solution file does not exist at the specified path!");
//             // You might want to throw an exception here or handle it,
//             // otherwise loadSolutionCVRPLib might crash or load nothing.
//         } else {
//             System.out.println("[DEBUG] File found successfully.");
//         }
        // --- DEBUG CODE END ---

        // Load the specific solution file structure
        try {
            referenceSolution.loadSolutionCVRPLib(solutionPath);

            // --- DEBUG CODE START: Check Loaded Data ---
//             System.out.println("[DEBUG] loadSolutionCVRPLib executed.");
//             System.out.println("[DEBUG] Reference Solution Cost (f): " + referenceSolution.f);
            // Optional: Print number of routes if your Solution class exposes it
            // System.out.println("[DEBUG] Routes: " + referenceSolution.routes.length);
            // --- DEBUG CODE END ---

        } catch (Exception e) {
            System.err.println("[DEBUG] EXCEPTION during solution loading: " + e.getMessage());
            e.printStackTrace();
        }
        ...
        // --- DEBUG CODE START: Final Check ---
//         System.out.println("[DEBUG] Best Solution Cloned. Initial BestF: " + bestF);
...
//         // --- DEBUG CODE END ---
        ...
        // --- NEW CONFIGURATION ---
        int debugPrintFrequency = 100; // Change this 'x' to whatever frequency you need
        // -------------------------
        ...
        while (!stoppingCriterion()) {
            ...
            // --- NEW DEBUGGING CODE ---
            if (print && iterator % debugPrintFrequency == 0) {
                // Assuming 'solution.getF()' or just 'solution.f' exists based on context.
                // If 'f' is a standalone variable updated in evaluateSolution(), use that instead.
                System.out.println("Iteration: " + iterator + " | Current f: " + solution.f + " | Best f: " + bestF);
            }
            // --------------------------
        }
