...
public class IntraLocalSearch 
{
	...
	public IntraLocalSearch(Instance instance,Config config)
	{
		...
		// limitAdj应该考虑instance的大小，避免访问KNN数组时越界
		// KNN数组大小是Math.min(config.getKnnLimit(), instance.getSize()-1)
		// 所以limitAdj也应该限制在instance.getSize()-1以内
		// 这与LocalSearch和FeasibilityPhase的处理方式一致
