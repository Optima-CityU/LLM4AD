...
from ails2_acc_continue import Ails2Evaluation         # TODO: 在服务器上跑的时候，修改这个地址为 ails2_acc_continue
from ails2_acc_continue import template_program
...
def main():
    ...
    task = Ails2Evaluation(dump_java_output_on_finish=False,
                           instances_subdir='XLDemo',
                           max_parallel_instances=8,
                           test_mode=True)                  # TODO: modify test_mode to False when running on the server

    method = EoH_Java(llm=llm,
                      profiler=EoH_java_Profiler(log_dir='logs', log_style='complex'),
                      evaluation=task,
                      max_sample_nums=20,               # TODO: modify max_sample_nums to 500 or 1000?
